...
class ShoppingListServiceSpec extends Specification {
    ...
    def "should save product into database"() {
        when:
        def product = new Product("beer", new Tag("drinks"), 3)
        ...
        productFromDB.tags.get(0).name == "drinks"
        productFromDB.price == 3
    }
    @Transactional
    def "should find all products from category"() {
        given:
        sut.saveProduct(new Product("bread", new Tag("breadstuff"), 2))
        sut.saveProduct(new Product("bun", new Tag("breadstuff"), 1))
        sut.saveProduct(new Product("ham", new Tag("meat"), 10))
        ...
        products.size() == 2
        products.each {
            assert it.tags.name.first() == "breadstuff"
        }
    }

    def "should update product price"() {
        given:
        def beer = new Product("beer", new Tag("drinks"), 3)
        sut.saveProduct(beer)
        def wine = new Product("wine", new Tag("drinks"), 20)
